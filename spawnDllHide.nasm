...
section .text
	...
	_WinMain@16:
		...
		; At this point Dll is loaded and his entry registered in PEB
		mov  edx, dword [esp+0x10]
		mov  dword [esp], edx
		call HideDll
		cmp  eax, 0
	...
	; int HideDll(UNICODE_STRING* module_to_hide_name);
	; *bFlink -> *hideFlink -> *fFlink 
	; *bBlink <- *hideBlink <- *fBlink
	; We need to patch *bFlink with *hideFlink and *fBlink by *hideBlink
	...
	HideDll:
		...
		mov  ebp, esp
		sub  esp, 0x10
		mov  esi, dword [esp+0x14] 		; UNICODE_STRING* module_to_hide_name
		mov  dword [esp], esi
		call GetPebEntry
		cmp  eax, 0
		je   HideDllError
		lea  edx, [eax+0x08*3] 			; mov _LDR_DATA_TABLE_ENTRY links limit

		HideDllLoop:
		; Now we got our module to hide entry address in EAX, let's patch his LIST_ENTRY structure
			mov  esi, dword [eax+0x00] 		; mov *hideFlink in ESI
			mov  ebx, dword [eax+0x04] 		; mov *hideBlink is EBX
			mov  dword [ebx], esi      		; patch *bFlink with *hideFlink
			mov  dword [esi+0x04], ebx	; patch *fBlink with *hideBlink
			add  eax, 0x08 ; LIST_ENTRY* ++
			cmp  eax, edx
			jne  HideDllLoop
